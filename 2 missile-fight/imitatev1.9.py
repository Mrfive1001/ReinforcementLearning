...
class Player(object):
    ...
    # strategy 策略是2的时候固定策略，是3的时候人类进攻
    # fool为1的时候可以打对面的空基地
    # 名字是为了后面的继承准备
    def __init__(self, symbol, strategy=1, fool=True, name='red', fight=False):
        self.symbol = symbol
        self.states_actions = {}
    ...
    def save_policy(self):
        with open('policy_1.9' + self.name, 'wb') as f:
            pickle.dump(self.states_actions, f)

    def load_policy(self):
        with open('policy_1.9' + self.name, 'rb') as f:
...
def humanfight(player1, player2):
    player1.reset()
    player2.reset()
    state = State()
    flag = state.is_end()
    damages = [-np.random.random(), -np.random.random()]
    while flag == 0:
        print('当前状态是', state.data)
        print('基地伤害分别是', damages)
        player1.feed_state(state)
        player2.feed_state(state)
        actionA = player1.select_action()
        actionB = player2.select_action()
        print('电脑选择动作', actionA)
        print('你选择动作', actionB)
        if actionA is not None:
            state = player1.next_state_action(state, actionA)
        if actionB is not None:
...
human = Player(1, strategy=3, fool=False, name='blue')
# train(A, B)
# A = Player(0, strategy=1, fool=True, name='red')
# B = Player(1, strategy=0, fool=False, name='blue')
A.load_policy()

humanfight(A, human)
# title = '程序员'
# text = '您的程序跑完了，没有错误，恭喜恭喜!!'
